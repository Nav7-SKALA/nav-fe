...
const CommonForm = ({ pageType }: CommonFormProps) => {
  ...
  return (
    <FormWrapper>
      ...
      <NaviLogo />
      <InfoText>Navi에 오신 걸 환영합니다. {pageType === 'login' ? '로그인' : '회원'} 정보를 입력하세요</InfoText>
      {/* 로그인/회원가입 폼 */}
      <Form onSubmit={handleSubmit}>
        ...
        <Button type="submit">{pageType === 'login' ? '로그인' : '회원가입'}</Button>
      </Form>

      {/* 추가 메뉴 */}
      <LoginMenu>
        <ForgotPassword>
          {pageType === 'login' ? '비밀번호를 잊으셨나요?' : '* 모든 항목은 필수 입력 사항입니다'}
        </ForgotPassword>
        <SignUp onClick={() => navigate(pageType === 'login' ? '/signup' : '/login')}>
